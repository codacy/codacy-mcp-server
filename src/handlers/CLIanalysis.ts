...
export async function CLIanalysisHandler(args: { tool: string; format: string; output: string }): Promise<{ message: string }> {
    
    const latestReleaseTag = await getLatestReleaseTag();

    const codacyCliPath = os.homedir + "/Library/Caches/Codacy/codacy-cli-v2/" + latestReleaseTag + "/codacy-cli-v2"
